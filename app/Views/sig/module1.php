...
<body>
    ...
    <div id="sim-panel">
        ...
        <div class="sim-body">
            ...
            <div class="sim-row"><label>Année recensement</label>
                <select id="sim-annee">
                    <option value="">(dernier disponible)</option>
                </select>
            </div>
    ...
    <script>
        ...
        const LIBELLES_FR = {
            ...
            HOSPITAL: 'Hôpital',
            CLINIC: 'Clinique',
            DOCTOR: 'Médecins / centre médical',
            DENTIST: 'Dentiste',
        };

        function libelleFr(l) {
            return l ? (LIBELLES_FR[String(l).toUpperCase()] || l) : '';
        }
        ...
        function setType(id, actif) {
            id = String(id);
            if (actif) typesActifs.add(id);
            else typesActifs.delete(id);
        ...
        $('#recherche').on('input', () => {
            clearTimeout(deb);
            deb = setTimeout(appliquerFiltres, 200);
        });
        ...
        $('#basemap button').on('click', function() {
            ...
            if ($(this).data('base') === 'satellite') {
                map.removeLayer(fondPlan);
                fondSat.addTo(map);
            } else {
                map.removeLayer(fondSat);
                fondPlan.addTo(map);
            }
        });
        ...
        function loadAnneesRecensement() {
            return $.get(API_BASE + '/api/statistiques/annees-recensement').done(function(d) {
                ...
                if (Array.isArray(d)) d.forEach(row => {
                    if (row.annee) $sel.append(`<option value="${row.annee}">${row.annee}</option>`);
                });
            }).fail(() => {});
        }
                ...
                nearest.forEach(n => {
                    if (n.latitude && n.longitude) {
                        L.marker([parseFloat(n.latitude), parseFloat(n.longitude)], {
                                icon: pinIcon(n.couleur_carte || COULEUR_DEFAUT)
                            })
        ...
        $('#sim-run').on('click', function() {
            ...
            $.ajax({
                ...
                .always(function() {
                    $('#sim-run').prop('disabled', false).text('Lancer la simulation');
                });
        });
